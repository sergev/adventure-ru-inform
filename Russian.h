...
Constant LanguageVersion = "@<<Информ@>>: Русская версия 0.9.0, 2018-08-09";
...
for (i=2: i~=len: i++) {
	! (все заглавные -> в строчные)
	buffer->i = UtoL(buffer->i);

	! (все 'ё' -> в 'е')
	if (buffer->i == 'ё') buffer->i = 'е';
        }
...
[ MorM obj;
  ...
  !return IIF (obj has pluralname, "могут", "может");
...
[ LanguageLM n x1;
  ...
  Tell:    switch(n)
           {   1: "Беседы с собой -- занятие нездоровое.";
               2: 
                  if (noun has female) {
                    "Твоя собеседница никак не отреагировала.";
                  }
                  "Твой собеседник никак не отреагировал.";
           }
